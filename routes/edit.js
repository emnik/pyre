  ...
  if (schedule !== undefined && schedule !== null) { // propably unnessesary check...
    db.serialize(function () {
      ...
      if (profile_name === 'WEEKLY') {
        ...
        for (var k in newtimewindows) {
          db.run('INSERT INTO timetable (schedule_id, time_window_id) VALUES (?,?);', [schedule[0], newtimewindows[k]], function (err) {
            if (err) {
              console.error(err)
              return next(err)
            }
...
router.get('/:id', isRequestLocal, isAuthenticated, get_sensors, get_timewindows, get_schedule, get_timewindows_perday, function (req, res) {
  ...
  if (req.params.id === '1') { // CONSTANT MODE
    ...
    db.all('SELECT time_window.name, time_window.temp, time_window.sensor_ids FROM time_window LEFT JOIN timetable ON timetable.time_window_id = time_window.id LEFT JOIN schedule ON schedule.id = timetable.schedule_id LEFT JOIN profile ON profile.id = schedule.profile_id WHERE profile.id = 2 LIMIT 2;', function (err, rows) {
      ...
      req.data = rows
      if (rows[0].name === 'DAY_PROFILE') {
        req.dayTemp = rows[0]
      };
      if (rows[1].name === 'NIGHT_PROFILE') {
        req.nightTemp = rows[1]
      };
      console.log(rows)
      res.render('edit/daynight', {
        dayTemp: req.dayTemp,
        nightTemp: req.nightTemp,
        sensors: req.sensors,
        base_url: base_url,
        isLocal: req.isLocal,
        show_edit: show_edit
    ...
    res.render('edit/weekly', {
      sensors: req.sensors,
      schedule: req.schedule,
      timewindow: req.timewindows,
      timewindowsperday: req.timewindowsperday,
      base_url: base_url,
      isLocal: req.isLocal,
      show_edit: show_edit
    })
