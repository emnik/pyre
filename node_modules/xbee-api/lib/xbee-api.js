...
XBeeAPI.prototype.parseRaw = function(buffer) {
  ...
  /* To solve the RangeError thrown by the buffer: 
   * I observed that the problem is when the buffer length is more than 8 so I reject those buffers...
   * One can see this by adding a console.log(buffer.length). The non-errornous buffers have length of 8+5 
   * (Data come in two chunks) This fix - hack - is only for my data comming in!!!
   */
  // console.log(buffer.length);
  // if (buffer.length<=8){ //this is to prevent the RangeError thrown by buffer's index out of range
    for(var i = 0; i < buffer.length; i++) {
      S.b = buffer[i];
      // if (S.b === C.START_BYTE && S.waiting) {
      if (S.b === C.START_BYTE) { //Range Error general FIX
      ...
      if (!S.waiting) {
          ...
          if (S.buffer.length > S.offset) { //RangeError general FIX
          S.buffer.writeUInt8(S.b, S.offset++);
          } else {
      ...
      //console.log(util.inspect(S)); //for debugging
